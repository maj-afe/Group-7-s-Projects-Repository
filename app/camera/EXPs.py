...
CHANNELS = 1
BUFFER_MS = 100
BLOCKSIZE = int(SAMPLE_RATE * (BUFFER_MS / 1000))
...
t_start = time.time()
filter_x = OneEuroFilter(t_start, SCREEN_W / 2, min_cutoff=0.1, beta=0.01)
filter_y = OneEuroFilter(t_start, SCREEN_H / 2, min_cutoff=0.1, beta=0.01)

# Mouth Click Thresholds
DEFAULT_MOUTH_THRESHOLD = 0.0350
MOUTH_OPEN_THRESHOLD = DEFAULT_MOUTH_THRESHOLD
is_mouth_open = False
...
print(
    "[INFO] Wayland Session Active. Press 'c' to calibrate center, 'o' for high"
    " mouth threshold, 'r' to reset threshold."
)
...
try:
  while cam.isOpened():
    ...
    if results.face_landmarks:
      ...
      if key == ord("c"):
        ...
        MOUTH_OPEN_THRESHOLD = DEFAULT_MOUTH_THRESHOLD + 0.0050
        print(f"[CONFIG] High Mouth Threshold Set: {MOUTH_OPEN_THRESHOLD}")

      elif key == ord("r"):
        MOUTH_OPEN_THRESHOLD = DEFAULT_MOUTH_THRESHOLD
        print(f"[CONFIG] Mouth Threshold Reset: {MOUTH_OPEN_THRESHOLD}")
  ...
  landmarker.close()
  ui.close()
  print("[INFO] Cleanup complete. Virtual mouse unmounted.")
